...
try {
  for (const viewport of [
    ...
    await expect(page.locator(".species-entry")).toHaveCount(8);
    expect(
      await page
        .locator("#tab-register")
        .evaluate((el) => el.scrollWidth <= el.clientWidth),
    ).toBe(true);
    await page.screenshot({ path: `artifacts/register-${viewport.width}.png` });
    await page.click('[data-tab="guide"]');
    console.log("Guide selected");
    await page.selectOption("#graphics", "performance");
    ...
    await page.keyboard.press("j");
    await page.click('[data-tab="guide"]');
    await expect(page.locator("#graphics")).toHaveValue("performance");
    await expect(page.locator("#motion")).not.toBeChecked();
    await expect(page.locator("#sensitivity")).toHaveValue("0.006");
    await page.close();
    console.log(
      "PASS: viewport, settings, and reload persistence",
      viewport.width,
    );
  }
