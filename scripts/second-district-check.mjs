// Explicit post-first-badge save fixture, not a fresh-save campaign run.
// A LV5 starter gains two levels from the rival and two from the first leader.
// £340 = £100 initial + two £120 rewards; 11 carriers = 8 initial + 3 reward.
// Three medkits assumes four of the initial five were used, then two rewarded.
// Movement, interaction, purchases and battle commands use ordinary UI input.
// The read-only development debug surface supplies observations, never setters.
...
try {
  ...
  for (const [x, z] of waypoints) {
    ...
    if (x === -185) {
      await look(-250, -20, 2);
      await page.screenshot({ path: "artifacts/second-district-approach.png" });
      observations.push({
        event: "route-stats",
  ...
  await look(-233, -37, 3);
  await page.screenshot({ path: "artifacts/second-district-front.png" });
  await look(-233, -29, 1.5);
  await page.waitForFunction(
    () => window.__debug.target() === "gym1",
    undefined,
    { timeout: 5000 },
  );
  ...
  await ready();
  await page.screenshot({ path: "artifacts/second-district-battle.png" });
  for (let turn = 1; turn <= 45; turn++) {
    ...
    if (heal) {
      ...
      await page.locator("#move-buttons button").nth(1).click();
    }
    await ready();
    const obs = {
      event: "turn",
  ...
  await page.screenshot({ path: "artifacts/second-district-result.png" });
  ...
  expect(final.badges).toContain(1);
  ...
  await page.screenshot({ path: "artifacts/second-district-failure.png" });
  throw error;
} finally {
  await writeFile(
    "artifacts/second-district-observations.json",
